...
const SettingsModal: React.FC<SettingsModalProps> = ({ 
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-md bg-white border border-zinc-200 rounded-xl shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-300"
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-zinc-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-zinc-900 tracking-wide">系统设置</h2>
              <p className="text-[10px] text-zinc-700 uppercase tracking-widest font-mono">System Settings</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="text-zinc-700 hover:text-zinc-900 transition-colors p-1"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
              ...
              {currentApiKey ? (
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="text-[10px] text-zinc-700 hover:text-zinc-700 transition-colors"
                  >
                    {showKey ? '隐藏' : '显示'}
                  </button>
                </div>
              ) : (
                <div className="text-xs text-zinc-700">未配置</div>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="flex-1 py-3 bg-indigo-600 text-white font-bold uppercase tracking-widest text-xs rounded-lg hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {currentApiKey ? '更新' : '保存'} <ArrowRight className="w-3 h-3" />
            </button>
            <button
              onClick={onClose}
              className="px-6 py-3 bg-zinc-100 text-zinc-700 font-bold uppercase tracking-widest text-xs rounded-lg hover:bg-zinc-200 transition-colors"
            >
              取消
            </button>
          </div>

          <div className="flex items-center justify-center gap-2 text-[10px] text-zinc-800 font-mono pt-2 border-t border-zinc-200">
